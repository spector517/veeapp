...
| Deploy | `deploy/` | `.deb` packaging, Nginx, Fail2ban, systemd, self-signed TLS (openssl) |
...
- Nginx handles TLS termination (self-signed EC P-256 cert, auto-generated on install into `/etc/default/veeapp/ssl/`, port 8443) and Basic Auth (`.htpasswd`).
- Fail2ban protects against brute-force (5 attempts / 10 min → 30 min ban; repeat → 7 day ban).
- `.deb` package depends on: `nginx`, `fail2ban`, `openjdk-25-jre-headless`, `apache2-utils`, `openssl`, `veepeenet (>= 2.4.2)`.
